...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function groupSkillsByType(skills) {
        ...
        skills.forEach(s => {
            ...
            if (!typeToSkills[type]) typeToSkills[type] = [];
            typeToSkills[type].push(s);
        });
        return typeToSkills;
    }

    function buildSkillItem(name, levelPercent) {
        ...
        progress.setAttribute('data-width', `${Math.max(0, Math.min(100, levelPercent))}%`);
    ...
    function renderSkills(skills) {
        ...
        Object.keys(grouped).forEach(type => {
            ...
            itemsWrap.className = 'skill-items';

            // 정렬: level 내림차순, 그 다음 name 오름차순
            grouped[type]
                .slice()
                .sort((a, b) => (b.level || 0) - (a.level || 0) || String(a.name).localeCompare(String(b.name)))
                .forEach(s => {
                    itemsWrap.appendChild(buildSkillItem(s.name, s.level));
                });

            category.appendChild(title);
            category.appendChild(itemsWrap);
            skillsRoot.appendChild(category);
        });

        // 새로 삽입된 스킬 바에 대해 관찰 재등록
        const newBars = document.querySelectorAll('#skills-root .skill-progress');
        newBars.forEach(bar => skillObserver.observe(bar));
    }

    function subscribeSkills() {
        if (!window.db || !window.__fire) return;
        const { collection, onSnapshot, query, orderBy } = window.__fire;
        try {
            const q = query(collection(window.db, 'skills'), orderBy('type'), orderBy('level', 'desc'), orderBy('name'));
            ...
            // 구버전 SDK 또는 규칙 오류 등은 콘솔에만 기록
            console.warn('Failed to subscribe Firestore skills:', e);
        }
    }

    // Firebase 초기화가 늦게 일어날 수 있어 재시도 훅 연결
    if (window.__initFirebase) window.__initFirebase();
    // 초기 시도
    subscribeSkills();
    // 딜레이 후 재시도 (초기 설정 지연 대비)
    setTimeout(subscribeSkills, 1500);
});
